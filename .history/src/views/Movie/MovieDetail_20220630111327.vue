    ...
    <div id="content" class="contentDetail">
		<div class="container" v-if="!isShowPhoto">
			...
			<div class="detail_list">
				<div class="detail-icon-back" @click="$router.back()">
						...
						<div class="inside">
							<ul>
								<li v-for="(item,index) in FileList.actors" :key="index">
									<div>
										<div class="actor-img">
											<img :src="item.avatarAddress" v-lazy="item.avatarAddress" alt="">
										</div>
			...
			<div class="movie-photo" v-if="FileList.photos">
				<div class="photo-bar">
					<span class="photo-bar-title">剧照</span>
					<span class="photo-all" @click="showMorePhoto()" >全部<span class="iconfont icon-jiantouyou"></span></span>
				</div>
				<div class="photo-content" ref="itemwrapper">
					<ul >
						<li v-for="(item2,index2) in FileList.photos" :key="index2">
							<div class="photo-single">
								<img :src="item2" v-lazy="item2" alt="">
							</div>	
						</li>
					</ul>
				</div>
		...
		<div class="show-photo" v-if="isShowPhoto">
			...
			<div class="show-photo-back" @click="showPhotoBack()">
						<span class="iconfont icon-back"></span>
			</div>
			<div class="photo-img">
				<div class="photo-item" v-for="(item3,index3) in FileList.photos" :key="index3">
					<img v-lazy="item3" alt="" @click="showThisImage()">
				</div>
			</div>
...
<script>
import { getCurrentInstance, onMounted, reactive, toRefs ,ref,nextTick,onBeforeMount} from 'vue'
...
export default {
     name:'MovieDetail',
	 components:{Spread,MovieDetailHeaderVue,ImagePreview},
    setup (props,context) {
	  ...
	  const findFilmById=async()=>{
		  let result=await api.movieInfo.findViewById({filmId:route.params.id,k:1735369})
			Info.FileList=result.film 
			 ...
			 Info.wordDetail=Info.FileList.synopsis 
	...
	function showThisImage(){
		 ImagePreview(Info.FileList.photos);
	}

function showPhotoBack(){
	Info.isShowPhoto=false;
	nextTick(()=>{
	 scrollInit()
	})
}

let itemwrapper=ref(null)
 function scrollInit(){//初始化
          let scrollWrapper=new BScroll(proxy.$refs.itemwrapper,{
                 probeType: 3,
				 scrollX:true,
                click: true,//穿透点击事件
                bounce: true,
