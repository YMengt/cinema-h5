<template>
    <div>
        ...
        <div id="content">
			...
			<!-- <div class="cinema_body">
                ...
                <ul v-if="!isShowAppOrder">
					<li v-for="(item,index) in showCinemaList" :key="index">                                                                   
                            ...
                            <div class="apporder">
                                <span @click="$router.push({path:'/cinemaDetail/'+item.cinemaId})"> <span>2D通兑票</span>|<span>3D通兑票</span> </span>
                            </div>
                            <div class="address">
                                <span>{{item.address}}</span>
                                <span>{{Number(item.Distance).toFixed(2) }}km</span>
            ...
            <CinemaDetailVue :showCinemaList="showCinemaList" :isShowAppOrder="isShowAppOrder"></CinemaDetailVue>
		</div>
...
import CinemaDetailVue from "../../components/Comp/CinemaDetail.vue";
    export default {
         name:'Cinema',
        components:{Header,TabBar, DropdownMenu, DropdownItem,Cell,Grid, GridItem,CinemaDetailVue},
    setup () {
        let info=reactive({
            cinemaList:[],
            showCinemaList:[],
