...
<div class="m-content overflow-line" id="J_description">{{mes2}}</div>
    <transition enter-active-class="active2" leave-active-class="active2">
      <p type="button" class="btn-more" v-if="isShowMore" id="J_btnmore" @click="showmoreDesc($event)"><span class="iconfont icon-xiangxiajiantou"></span>
      </p>
    </transition>
    </div>
</template>
    ...
    setup (props) {
        ...
        introduce: ""
         ...
         info.introduce=props.mes2;
          if(info.introduce.length>75){//文字超过字数，开启展示更过
