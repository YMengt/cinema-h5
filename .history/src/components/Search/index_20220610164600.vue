...
<MovieDetailHeaderVue :isShowBack="true" :title="serachType"></MovieDetailHeaderVue>
				<div class="search_input">
					<div class="search_input_wrapper" >
						<i class="iconfont icon-sousuo" @click="clickSearch()"></i>
...
//点击搜索
			function clickSearch(){
				info.showList.length=0
				if(info.keyWords){
					info.dataList.forEach(item=>{
						let str=item.name
						if(new RegExp(info.keyWords).test(str)){
							info.showList.push(item)
						}
					})
