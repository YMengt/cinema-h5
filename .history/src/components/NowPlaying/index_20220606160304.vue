                    ...
                    <li v-for="(item,index) in movieList" :key="index" @click="MovieDeteil(item)">
                       <div class="pic_show"><img :src="item.poster"></div>
                        <div class="info_list">
                            ...
                            <p v-show="isPresale">观众评分 <span class="grade">{{item.grade}}</span></p>
                            <p>主演:{{filterName(item)}}</p>
...
function onLoad(){
        if( requestInfo.pageNum<Math.ceil(ListInfo.total/requestInfo.pageSize)){
