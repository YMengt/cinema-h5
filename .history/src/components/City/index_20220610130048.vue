        ...
        <div class="city_body">
				<div class="city_list">
					<div class="city_hot">
						<h2>热门城市</h2>
						<ul class="clearfix">
							<li v-for="(item,index) in hotCity" :key="index" @click="getCityId(item.cityId)">{{item.name}}</li>
				...
				<index-bar sticky=false :sticky-offset-top="44">
					...
					<index-anchor class="city-letter"  :index="item.letter"  />		<div class="city-list">	
						<cell class="city-single" v-for="(item2,index2) in item.value" :key="index2" :title="item2.name" @click="getCityId(item2.cityId)"></cell>
						</div>
					</div>
				</index-bar>
