    ...
    <div>
       ...
       <span  @click="cityBack()" class="iconfont icon-back"></span>  <div>当前城市-{{$store.state.cityInfo.name || ""}}</div>
        ...
        <div class="city_body">
				...
				<index-bar sticky="false" :sticky-offset-top="44" :index-list="letterList">
    ...
    export default {
		...
		setup(){
			...
			const getCity=async()=>{
				...
				info.sortProvince.sort((pre,cur)=>{////排序
					return pre.letter.localeCompare(cur.letter)
					//localeCompare用本地特定的顺序来比较两个字符串。
				})				
				// console.log(info.sortProvince)
				//右侧索引栏
				info.sortProvince.forEach(item=>{
					info.letterList.push(item.letter)		
				})
				// console.log(info.letterList)
			}
			getCity()
